...
const App: FC = () => {
  ...
  const [sortTiles, setSortTiles] = useState<
    { sort: string; sortBy: string } | undefined
  >();

  const createTile = () => {
    const time = new Date();
    setTileArr([
      { id: uuidv4(), title: "", description: "", time: time.toLocaleString() },
      ...(tileArr || []),
    ]);
  };
  ...
  const sortBy = (event) => {
    ...
    const titleAsc = [...(tileArr || [])].sort((a, b) =>
      a.title.toLowerCase() > b.title.toLowerCase() ? 1 : -1
    );
    const titleDesc = [...(tileArr || [])].sort((a, b) =>
      a.title.toLowerCase() > b.title.toLowerCase() ? -1 : 1
    );
    const dateAsc = [...(tileArr || [])].sort((a, b) =>
      a.time > b.time ? 1 : -1
    );
    const dateDesc = [...(tileArr || [])].sort((a, b) =>
      a.time > b.time ? -1 : 1
    );
      ...
      <div className="inner-container scale">
        ...
        <div className="header-btn-container">
          ...
          <div className="sorting-container">
            <button name="date" onClick={(event) => sortBy(event)}>
              Sort by Date
            </button>
            <div className="sorting-icon-cont">
              {sortTiles &&
                (sortTiles.sortBy === "date" && sortTiles.sort === "desc" ? (
                  <img src={desc_icon} alt="" />
                ) : sortTiles.sortBy === "date" && sortTiles.sort === "asc" ? (
                  <img src={asc_icon} alt="" />
                ) : (
                  ""
                ))}
            </div>
            <button name="title" onClick={(event) => sortBy(event)}>
              Sort by Title
            </button>
            <div className="sorting-icon-cont">
              {sortTiles &&
                (sortTiles.sortBy === "title" && sortTiles.sort === "desc" ? (
                  <img src={desc_icon} alt="" />
                ) : sortTiles.sortBy === "title" && sortTiles.sort === "asc" ? (
                  <img src={asc_icon} alt="" />
                ) : (
                  ""
                ))}
            ...
            {typeof tileArr === "object" &&
              tileArr.map((tile) => (
                <div className="tile slide-in" key={tile.id}>
                  <input
                    onChange={(event) => editTile(tile.id, event)}
                    name="title"
                    type="text"
                  ...
                  <div className="desc-length-cont">
                    {tile.description.length > 89 && (
                      <span>{140 - tile.description.length}</span>
                    )}
                  </div>
